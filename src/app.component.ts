...
@Component({
  ...
  providers: [provideMarkdown({ katex: true })],
...
export class AppComponent {
  ...
  // Completed Tasks & Notes (Block Notes - 舊有區塊筆記)
  ...
  // --- 新增：知識卡片筆記系統 (Task Notes - 任務專屬筆記) ---
  taskNotes = signal<Record<string, string>>({}); 
  activeTask = signal<string | null>(null); 
  ...
  detailedSchedule: { [key: number]: DailyTask[] } = {
    ...
    // ... (其他週的數據保持不變，為節省篇幅此處省略，請保留您原本的數據) ... 
    // 若您貼上時覆蓋了數據，請確保您的 WeeksData 2-12 還在。
    // 如果需要我提供完整的 12 週數據請告訴我，但通常您只需要替換 class 內部的邏輯部分。
    // 為了安全起見，下面我會包含完整的 WeeksData 結構，請檢查。
    2: [
    ...
    12: [
       ...
       { day_id: "W12D5", title: "最後總整理與任務達成", am: { topic: "面試 Checkbox 複習", tasks: ["確保隨時可展示簡報代碼", "總結學習歷程與感謝", "複習關鍵數學公式"] }, pm: { topic: "社群連結與反饋", tasks: ["整理面試反饋 (如有)", "保持市場關注", "參與討論
